...
export function createBrowserApi(): ConstellationApi {
  ...
  return {
    auth: { getState: async () => ({ status: 'authorized', login: 'mode-demo' }), accounts: async () => [{ login: 'mode-demo', avatarUrl: '', active: true }], switch: async () => ({ status: 'authorized', login: 'mode-demo' }), startDeviceFlow: async () => ({ status: 'denied', error: 'La connexion GitHub réelle est disponible dans la fenêtre Electron.' }), logout: async () => undefined },
